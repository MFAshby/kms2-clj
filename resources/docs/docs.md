...
The `home-routes` handler in the `kms2-clj.routes.home` namespace
...
The `home-page` function will in turn call the `kms2-clj.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `kms2-clj.handler` namespace:
...
Request middleware functions are located under the `kms2-clj.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `kms2-clj.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `kms2-clj.db.core` in some other namespace.
